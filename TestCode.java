...
public class TestCode extends LinearOpMode {
    ...
    private void performAction(String actionName, int port, int i2cAddress, int memAddress, int memLength) {
        if (actionName.equalsIgnoreCase("read"))
            dim.enableI2cReadMode(port, I2cAddr.create7bit(i2cAddress), memAddress, memLength);
        if (actionName.equalsIgnoreCase("write"))
            dim.enableI2cWriteMode(port, I2cAddr.create7bit(i2cAddress), memAddress, memLength);
